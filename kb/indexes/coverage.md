...
| `wiki/topics/tls-fingerprint-mitigation.md` | concept | `nba_api` `stats/library/http.py` and `library/http.py` (installed `.venv`, exact tag `v1.11.4`), curl_cffi/curl-impersonate/httpx/tls-client PyPI + GitHub metadata, official Cloudflare/Akamai/JA3/JA4 docs, `nba_api` GitHub issue tracker | active | 2026-09-19 | ranked research on replacing/wrapping `NBAStatsHTTP`'s `requests.Session` transport to defeat the diagnosed TLS/HTTP2 fingerprint block; recommends `curl_cffi` via `requests.Session(impersonate=...)`; no implementation performed |
