...
| `wiki/operations/full-extraction-requirements.md` | concept | `full-extraction.yml`, `nordvpn-connect` action source, `AGENTS.md`, Mullvad/NordVPN primary sources, fork API observation, run history (`35428015854`, `35428089035`, `35430687683`, Debug NBA Probe runs, `35469167028`, `35484551473`, `35487068911`), `endpoint_coverage.py`, `nba_api_adapter.py`, `rebind_star_semantic_decision_corpora.py`, `agg_all_time_leaders.py`, `nba_api/stats/library/http.py` | active | 2026-09-20 | CI dispatch prerequisites plus plan-gate contract, plus diagnosed root cause of the `extract`-stage hang: NBA Stats API TLS/HTTP fingerprint-blocks Python `requests`/urllib3 (curl succeeds on the same tunnel/exit IP/endpoint every time) — blocks all 162 extractors over VPN pending a transport-layer decision; transport swapped to curl_cffi, after which the preflight canary's `empty` rejection was traced to a `commonallplayers` header permutation (not a VPN block) and the pinned contracts were found stale against live NBA on four further endpoints that upstream `nba_api` v1.11.4 does not document; extraction now reaches the runner and every lane dies on the MODEL-GREEN assurance gate, which is unreachable by authoring: `model_green` is unconditionally false, four of six blocking authorities have no admission seam at all, and the rest need an independent second party |
| `wiki/topics/tls-fingerprint-mitigation.md` | concept | `nba_api` `stats/library/http.py` and `library/http.py` (installed `.venv`, exact tag `v1.11.4`), curl_cffi/curl-impersonate/httpx/tls-client PyPI + GitHub metadata, official Cloudflare/Akamai/JA3/JA4 docs, `nba_api` GitHub issue tracker | active | 2026-09-19 | implemented `curl_cffi` transport swap (2026-09-19): `_PinnedTransportSession` with `impersonate="chrome"`, requests-compatible query encoding, in-code no-ambient-proxy enforcement, certifi-pinned CA trust, and curl-transport fault classification; live CI confirmation pending |
